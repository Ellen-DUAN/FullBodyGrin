...
export default class PlanNewWorkout extends Component {
    ...
    handleSubmit = () => {
        ...
        if(this.state.completion_date === null || this.state.completion_date === undefined) {
            ...
            this.setState({ message: 'Completion date cannot be before today!' });
            this.setState({ show: true });
            ...
            Axios.post('/useRecommendation',
                ...
                res => {
                    if(res.data.saved) {
                        this.setState({ message: "Workout recommendation has been used successfully!" });
                        this.setState({ show: true });
                    }
    ...
    componentDidMount() {
        ...
        console.log('&&&&&&&&&&&');
        console.log(this.props.location.recommendationjson);
        const recommendation = this.props.location.recommendationjson;
        if(recommendation !== undefined) {
            this.setState({ workout_type: recommendation['workout_type'] });
            this.setState({ time_taken: recommendation['time'] });
            this.setState({ recommended_distance: recommendation['distance'] });
        } else {
            Axios.get(
                '/getRecommendation/id=' + this.props.match.params.id
            ).then(
                res => {
                    const re = res.data.recommendation;
                    this.setState({ workout_type: re['workout_type'] });
                    this.setState({ time_taken: re['time'] });
                    this.setState({ recommended_distance: re['distance'] });
                }
            );
        }
    ...
    render() {
        ...
        <div>
            ...
            <Form id="new_workout_form">
                ...
                <Form.Group>
                    <Form.Label>Recommended Distance (km): {this.state.recommended_distance}</Form.Label>
                </Form.Group>
                <Form.Group>
                ...
                <Form.Group>
                    ...
                    <Form.Control disabled value={this.state.time_taken} type="number"
